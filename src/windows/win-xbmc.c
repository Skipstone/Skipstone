...
void win_xbmc_init(void) {
	controlling_type = persist_read_int(KEY_PERSIST_XBMC_CONTROLLING_TYPE);
...
void win_xbmc_deinit(void) {
	persist_write_int(KEY_PERSIST_XBMC_CONTROLLING_TYPE, controlling_type);
...
static void update_timer_callback(void *data) {
	xbmc_request(KEY_REQUEST_REFRESH);
...
static void down_long_click_handler(ClickRecognizerRef recognizer, void *context) {
	switch (controlling_type) {
		case CONTROLLING_TYPE_KEYPAD:
			xbmc_request(KEY_REQUEST_RIGHT);
			break;
		case CONTROLLING_TYPE_SEEK:
